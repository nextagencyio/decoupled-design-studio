...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/work/${slug.join('/')}`
  const item = await getProject(path)

  if (!item) {
    return { title: 'Project Not Found | Design Studio' }
  }

  return {
    title: `${item.title} | Design Studio`,
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  ...
                  {(item as any).completionYear && (
                    <div>
                      <dt className="text-sm text-gray-500">Completion Year</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).completionYear}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
